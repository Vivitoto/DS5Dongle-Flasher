...
function getProxyUrlFromEnv(env) {
  return envValue(env, ['GITHUB_PROXY', 'github_proxy', 'HTTPS_PROXY', 'https_proxy', 'HTTP_PROXY', 'http_proxy', 'ALL_PROXY', 'all_proxy']);
}

function parseProxyDirective(proxyDirective) {
  const directive = String(proxyDirective || '').trim();
  if (!directive || /^DIRECT$/i.test(directive)) {
    return '';
  }

  const parts = directive.split(';').map((item) => item.trim()).filter(Boolean);
  for (const part of parts) {
    ...
    const match = part.match(/^(PROXY|HTTPS|HTTP|SOCKS|SOCKS5)\s+(.+)$/i);
    if (!match) {
      continue;
    }
...
async function resolveProxyUrl(urlString, env, proxyResolver) {
  const envProxy = getProxyUrlFromEnv(env);
  if (envProxy) {
    return envProxy;
  }

  if (typeof proxyResolver !== 'function') {
    return '';
  }

  try {
    return parseProxyDirective(await proxyResolver(urlString));
  } catch {
    return '';
  }
}

export async function resolveProxyStatus(urlString, env = process.env, proxyResolver = null) {
  if (shouldBypassProxy(urlString, env)) {
    return { mode: 'bypass', proxyUrl: '', source: 'no_proxy' };
  }

  const envProxy = getProxyUrlFromEnv(env);
  if (envProxy) {
    return { mode: 'proxy', proxyUrl: envProxy, source: 'env' };
  }

  if (typeof proxyResolver !== 'function') {
    return { mode: 'direct', proxyUrl: '', source: 'none' };
  }

  try {
    const proxyUrl = parseProxyDirective(await proxyResolver(urlString));
    if (proxyUrl) {
      return { mode: 'proxy', proxyUrl, source: 'system' };
    }
  } catch {
...
async function getDispatcher(urlString, env, proxyResolver) {
  const status = await resolveProxyStatus(urlString, env, proxyResolver);
  if (status.mode !== 'proxy') {
    return undefined;
  }

  if (!proxyCache.has(status.proxyUrl)) {
    proxyCache.set(status.proxyUrl, new ProxyAgent(status.proxyUrl));
  }

  return proxyCache.get(status.proxyUrl);
}

async function githubFetch(url, env, init = {}, proxyResolver = null) {
  ...
  const response = await fetch(url, {
    ...init,
    headers,
    dispatcher: await getDispatcher(url, env, proxyResolver),
  });
...
export function resolveRepoConfig(env = process.env) {
  ...
  return {
    owner,
    repo,
    dataDir: envValue(env, ['DATA_DIR', 'data_dir']) || '/app/data',
    port: Number(envValue(env, ['PORT', 'port']) || 3000),
  };
}

export function resolveStorageConfig(env = process.env, config = resolveRepoConfig(env)) {
  const userDataDir = envValue(env, ['APPDATA', 'LOCALAPPDATA', 'XDG_DATA_HOME']) || config.dataDir;
  const dataDir = envValue(env, ['DATA_DIR', 'data_dir']) || path.join(userDataDir, 'DS5Dongle-Flasher', 'data');
...
export async function fetchReleases(config, env = process.env, proxyResolver = null) {
  const url = `https://api.github.com/repos/${encodeURIComponent(config.owner)}/${encodeURIComponent(config.repo)}/releases?per_page=30`;
  const response = await githubFetch(url, env, {}, proxyResolver);
...
export async function prepareReleasePackage({ config, tag, variant, env = process.env, log = () => {}, progress = () => {}, proxyResolver = null }) {
...
export { parseProxyDirective, resolveProxyUrl };
