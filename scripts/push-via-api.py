...
This is DS5Dongle Flasher's durable workaround for this workstation, where
normal `git push`/`git fetch` over HTTPS can time out even though `gh api`
works.

Default behavior:
- Pushes local commits to Vivitoto/DS5Dongle-Flasher main via GitHub's Git Data API.
...
Environment overrides:
  DS5DONGLE_FLASHER_REPO=owner/repo
  DS5DONGLE_FLASHER_BRANCH=main
"""
...
DEFAULT_REPO = os.environ.get("DS5DONGLE_FLASHER_REPO", "Vivitoto/DS5Dongle-Flasher")
DEFAULT_BRANCH = os.environ.get("DS5DONGLE_FLASHER_BRANCH", "main")
...
def main() -> int:
    ...
    try:
        ...
        remote_sha = None
        remote_tree = "4b825dc642cb6eb9a060e54bf8d69288fbee4904"
    ...
    print(f"replay base   {base}")
    print(f"replay count  {len(replay)}")
    if ARGS.dry_run:
        print("dry-run: not creating blobs/commits or moving refs")
        return 0
